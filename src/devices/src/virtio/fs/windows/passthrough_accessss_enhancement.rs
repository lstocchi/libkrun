...
use windows_sys::Wdk::Storage::FileSystem::{
    ...
    FileIdBothDirectoryInformation, FILE_ACCESS_INFORMATION, FileAccessInformation, NtQueryInformationFile,
};
...
pub fn is_handle_read_only(handle: u64) -> bool {
    // Safely ignore your dummy init handle and directory tokens
...
impl PassthroughFs {
    ...
    fn fill_dir_cache(&self, inode: Inode, handle: Handle) -> io::Result<()> {
        ...
        let dir_g = HandleGuard(open_by_id(root_h, inode_data.file_index, &dir_flags)
        ...
        self.dir_caches.write().unwrap().insert(handle, Mutex::new(entries));
        Ok(())
    }
}
...
impl FileSystem for PassthroughFs {
    ...
    fn init(&self, capable: FsOptions) -> io::Result<FsOptions> {
        ...
        let file_info =
            get_file_info(&root_path).map_err(win_err_to_linux)?;
    ...
    fn setattr(
        ...
        if valid.contains(SetattrValid::SIZE) {
            if let Some(h) = handle {
                ...
                let file = self.reopen_inode(inode, h, GENERIC_READ | GENERIC_WRITE).map_err(win_err_to_linux)?;
    ...
    */
    ...
    fn read<W: io::Write + ZeroCopyWriter>(
        ...
        let file = self.reopen_inode(inode, handle, GENERIC_READ).map_err(win_err_to_linux)?;
    ...
    fn lseek(
        ...
        match whence {
            ...
            2 => {                                   // SEEK_END
                let file = self.reopen_inode(inode, handle, FILE_READ_ATTRIBUTES).map_err(win_err_to_linux)?;
                let len = file.metadata().map_err(win_err_to_linux)?.len();
                Ok((len as i64 + offset as i64) as u64)
            }
            3 => Ok(offset),                        // SEEK_DATA
            4 => {                                   // SEEK_HOLE — return EOF
                let file = self.reopen_inode(inode, handle, FILE_READ_ATTRIBUTES).map_err(win_err_to_linux)?;
                let len = file.metadata().map_err(win_err_to_linux)?.len();
                Ok(len)
            }
