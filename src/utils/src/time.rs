...
#[cfg(target_os = "windows")]
use windows_sys::Win32::{Foundation::HANDLE, System::SystemInformation::GetSystemTimePreciseAsFileTime};
#[cfg(target_os = "windows")]
...
pub fn get_time(clock_type: ClockType) -> u64 {
    match clock_type {
        ...
        ClockType::ProcessCpu => unsafe {
            get_handle_cpu_time(GetCurrentProcess(), GetProcessTimes)
        },
        ClockType::ThreadCpu => unsafe {
            get_handle_cpu_time(GetCurrentThread(), GetThreadTimes)
        },
    }
}

#[cfg(target_os = "windows")]
unsafe fn get_handle_cpu_time(
    handle: HANDLE,
    time_fn: unsafe extern "system" fn(
        HANDLE,
        *mut FILETIME,
        *mut FILETIME,
        *mut FILETIME,
